        ...
        <div class=" input-group mb-3 p-0">
            <form action="?controller=product&action=search" method="POST" class=" d-flex container">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="admin.php?controller=product">Tất cả sản phẩm</a></li>
                    <?php while($dm = mysqli_fetch_array($category)): ?>
                    <li>
                        <a class="dropdown-item" href="admin.php?controller=product&sort=<?php echo $dm['idDanhMuc'] ?>"><?php echo 'Lọc theo danh mục: '.$dm['tenDanhMuc'] ?></a>
                    </li>
                    <?php endwhile; ?>
                </ul>
            ...
            <tr>
                ...
                <td>
                        ...
                        <button type="button" class="btn btn-danger" id="xoa">
                            <a  class= "text-white" onclick="confirmDelete(event)" href="admin.php?controller=product&action=changeDisplay&hienThi=0&id=<?php echo $sp['idSanPham'] ?>">
        ...
        <div class="row justify-content-center">
            <div class="col-2">
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        <li class="page-item">
                            <a class="page-link" href="admin.php?controller=product&<?php if(isset($_GET['sort'])){echo 'sort='.$sort.'&';}?>page=<?php $move = $page - 1;if($move > 1){echo $move;}else{echo 1;} ?>"
                            aria-label="Previous">
                        ...
                        ?>
                            <li class="page-item">
                                <a class="page-link <?php if($i == $page){echo 'text-bg-primary';} ?>" 
                                    href="admin.php?controller=product&<?php if(isset($_GET['sort'])){echo 'sort='.$sort.'&';}?>page=<?php echo $i; ?>"><?php echo $i; ?>
                                </a>
                            </li>
                        <?php
                            }
                        ?>
                        <li class="page-item">
                            <a class="page-link" href="admin.php?controller=product&<?php if(isset($_GET['sort'])){echo 'sort='.$sort.'&';}?>page=<?php $move = $page + 1;if($move <= $trang){echo $move;}else{echo $trang;} ?>" 
                            aria-label="Next">
